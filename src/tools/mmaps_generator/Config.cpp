...
namespace MMAP
{
    static float ComputeBaseUnitDim(int vertexPerMapEdge)
    {
        return GRID_SIZE / static_cast<float>(vertexPerMapEdge);
    }

    static std::pair<uint32, uint32> MakeTileKey(uint32 x, uint32 y)
    {
        return { x, y };
    }

    static bool isCurrentDirectory(const std::string& pathStr)
    {
        try
        {
    ...
    bool Config::LoadConfig(std::string_view configFile)
    {
        ...
        if (!mmapsNode.contains("meshSettings"))
            return true;
        ...
        // Fallback: if dataDir is "current dir" but no maps/ folder is found there,
        // try the executable's directory (the typical case when running mmaps_generator
        // from a bin/ folder containing maps/, vmaps/, dbc/...).
