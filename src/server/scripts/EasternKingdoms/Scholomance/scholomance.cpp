    ...
    struct npc_scholomance_boneweaverAI : public ScriptedAI
    {
        ...
        void JustDied(Unit* who)
        {
            if (InstanceScript* pInstance = me->GetInstanceScript())
                if (Creature* pRattlegore = ObjectAccessor::GetCreature(*me, pInstance->GetGuidData(DATA_RATTLEGORE)))
            ...
            if (InstanceScript* pInstance = me->GetInstanceScript())
            {
                if (Aura const* aur = me->GetAura(SPELL_BOILING_BLOODTHRIST))
                    if (aur->GetStackAmount() >= 99)
                        pInstance->DoUpdateAchievementCriteria(CRITERIA_TYPE_BE_SPELL_TARGET2, SPELL_SANGUINARIAN, 0, me);
            }
