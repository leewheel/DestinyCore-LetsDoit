...
public:
    ...
    struct boss_lilian_vossAI : public BossAI
    {
        ...
        void DamageTaken(Unit* /*attacker*/, uint32& damage)
        {
            ...
            {
                phase = 1;
                if (Creature* pGadling = ObjectAccessor::GetCreature(*me, instance->GetGuidData(DATA_GANDLING_EVENT)))
    ...
    private:
        ...
        void EndEncounter()
        {
            ...
            if (IsHeroic())
            {
                instance->DoRespawnGameObject(instance->GetGuidData(DATA_COFFER_OF_FORGOTTEN_SOULS_HEROIC));
            }
            else
            {
                instance->DoRespawnGameObject(instance->GetGuidData(DATA_COFFER_OF_FORGOTTEN_SOULS));
            }
        }
    };
};
        ...
        void DamageTaken(Unit* /*who*/, uint32& damage)
        {
            if (damage >= me->GetHealth())
            {
                ...
                if (!isPhase)
                {
                    ...
                    if (Creature* pGandling = ObjectAccessor::GetCreature(*me, instance->GetGuidData(DATA_GANDLING_EVENT)))
                    {
        ...
        void UpdateAI(uint32 const diff)
        {
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_GANDLING_INTRO_1:
                    ...
                    if (Creature* pLilian = ObjectAccessor::GetCreature(*me, pInstance->GetGuidData(DATA_LILLIAN_VOSS)))
                    {
    ...
    bool OnTrigger(Player* player, AreaTriggerEntry const* /*areaTrigger*/, bool p_Enter)
    {
        if (InstanceScript* pInstance = player->GetInstanceScript())
        {
            if (pInstance->GetData(DATA_LILIAN_ENTRANCE) == NOT_STARTED)
            {
                ...
                if (Creature* pGandling = ObjectAccessor::GetCreature(*player, pInstance->GetGuidData(DATA_GANDLING_EVENT)))
