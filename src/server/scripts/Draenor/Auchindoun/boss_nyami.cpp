        ...
        case EventOnDeath3:
        {
            if (instance)
            {
                ...
                if (Creature* l_Warden = me->GetSummonedCreatureByEntry(CreatureWardenAzzakael))
                {
                    ...
                    l_Warden->SetFlag(UNIT_FIELD_FLAGS, UnitFlags::UNIT_FLAG_REMOVE_CLIENT_CONTROL);
                    l_Warden->SetFlag(OBJECT_DYNAMIC_FLAGS, UNIT_DYNFLAG_DEAD);
                    l_Warden->SetFlag(UNIT_FIELD_FLAGS, UnitFlags2::UNIT_FLAG2_DISABLE_TURN | UnitFlags2::UNIT_FLAG2_FEIGN_DEATH);
