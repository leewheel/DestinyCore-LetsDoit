        ...
        struct boss_oregorgerAI : public BossAI
        {
            ...
            void Reset() override
            {
               // ClearDelayedOperations();
                ...
                {
                    l_Ore->Respawn();
                    l_Ore->SetFlag(GAMEOBJECT_FLAGS, GO_FLAG_NOT_SELECTABLE);
                }
            ...
            void JustSummoned(Creature* p_Summon) override
            {
                switch (p_Summon->GetEntry())
                {
                    ...
                    {
                        p_Summon->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_REMOVE_CLIENT_CONTROL);
                        break;
                    }
                    case eCreatures::BlackrockOre:
                    {
                        ...
                        p_Summon->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_REMOVE_CLIENT_CONTROL | UnitFlags::UNIT_FLAG_IMMUNE_TO_PC | UnitFlags::UNIT_FLAG_NOT_SELECTABLE);
                        p_Summon->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG2_DISABLE_TURN);
                        break;
                    }
            ...
            void SetPower(Powers p_Power, int32 p_Value)
            {
                ...
                if (p_Value == 0 && m_Phase == ePhases::PhaseFight)
                {
                    ...
                    if (!m_VolatileOre && m_Instance != nullptr)
                    {
                        ...
                        if (GameObject* l_Ore = ObjectAccessor::GetGameObject(*me, m_Instance->GetGuidData(eFoundryGameObjects::VolatileBlackrockOre)))
                            l_Ore->RemoveFlag(GAMEOBJECT_FLAGS, GO_FLAG_NOT_SELECTABLE);
                    }
                  ...
                  //  {
                        for (uint8 l_I : m_CratesToActivate)
                        {
                            if (Creature* l_Crate = me->SummonCreature(eCreatures::OreCrate, g_OreCrateSpawnPos[l_I]))
                            {
                                ...
                                l_Crate->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG2_DISABLE_TURN);
